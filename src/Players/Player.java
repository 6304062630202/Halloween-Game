...
public class Player extends MapObject {
	...
	// animations
	private ArrayList<BufferedImage[]> bone;
	...
	public Player(TileMap tm) {
		...
		try {
			...
			int count = 0;
			bone = new ArrayList<BufferedImage[]>();
		...
		Studio.load("/sounds/playerjump.mp3", "playerjump");
		Studio.load("/sounds/playerlands.mp3", "playerlands");
		Studio.load("/sounds/playerattack.mp3", "playerattack");
		Studio.load("/sounds/playerhit.mp3", "playerhit");
		Studio.load("/sounds/playercharge.mp3", "playercharge");
	...
	private void setAnimation(int i) {
		currentAction = i;
		frame.setFrames(bone.get(currentAction));
		frame.setDelay(SPRITEDELAYS[currentAction]);
	...
	public void update() {
		...
		for(int i = 0; i < enemies.size(); i++) {
			...
			// check attack
			if(currentAction == ATTACKING &&
					frame.getFrame() == 3 && frame.getCount() == 0) {
				if(e.intersects(ar)) {
					e.hit(damage);
				}
			}
		...
		if(teleporting) {
			...
			if(currentAction != UPATTACKING) {
				Studio.play("playerattack");
				setAnimation(UPATTACKING);
				aur.x = (int)x - 15;
				aur.y = (int)y - 50;
			}
			else {
				if(frame.getFrame() == 4 && frame.getCount() == 0) {
					for(int c = 0; c < 3; c++) {
						energyParticles.add(
							new EnergyParticle(
								tileMap,
